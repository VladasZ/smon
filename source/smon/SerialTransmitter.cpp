...
SerialMonitorOld::SerialMonitorOld(const string& port, unsigned baud_rate) {
    stop = false;
    io = new io_service();
...
SerialMonitorOld::~SerialMonitorOld() {
#ifdef SMON_DONT_THROW_ON_CONNECTION_ERRORS
...
bool SerialMonitorOld::has_data() {
    return !received_packets.empty();
}

void SerialMonitorOld::_read(void* buf, unsigned size, uint16_t id) {
...
void SerialMonitorOld::_write(const void* buf, unsigned size) {
#ifdef SMON_DONT_THROW_ON_CONNECTION_ERRORS
    if (failed_init) return;
#endif
    mutex.lock();
